...
export default function ProjectTile({ project, onClick, isEditable = false }: ProjectTileProps) {
  ...
  if (!project?.id || !project?.name) {
    return (
      <Card className="group h-full dark:bg-zinc-900/90 bg-gray-100/90 border-gray-300">
        <CardContent className="p-6">
          <div className="space-y-4">
            <div>
              <h3 className="text-xl font-semibold dark:text-zinc-100 text-gray-700">
  ...
  if (isEditable) {
    return (
      <Card 
        className="group hover:shadow-lg transition-all duration-200 cursor-pointer h-full dark:bg-zinc-900/90 dark:hover:bg-zinc-900 bg-gray-100/90 hover:bg-gray-200/90 border-gray-300"
        onClick={onClick}
      >
        <CardContent className="p-6 flex flex-col justify-between h-full">
          ...
          {project.imageUrl && (
            <div className="mb-4 w-full h-48 relative overflow-hidden rounded-lg">
              <div className="absolute inset-0 bg-gradient-to-t dark:from-zinc-900 dark:via-zinc-900/50 from-gray-200/90 via-gray-100/50 to-transparent z-10" />
              <img
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger>
                      <h3 className="text-xl font-semibold dark:text-zinc-100 text-gray-700 group-hover:text-primary/90 truncate max-w-[200px]">
            ...
            <div className="flex flex-wrap gap-2 mt-2 mb-4">
              {project.techStack?.map((tech, index) => (
                <Badge 
                  key={index} 
                  variant="secondary"
                  className="dark:bg-zinc-800 dark:text-zinc-200 bg-gray-300/90 text-gray-700"
                >
                  {tech}
                </Badge>
              ))}
  ...
  return (
    <Link href={projectLink}>
      <Card 
        className="group hover:shadow-lg transition-all duration-200 cursor-pointer h-full dark:bg-zinc-900/90 dark:hover:bg-zinc-900 bg-gray-100/90 hover:bg-gray-200/90 "
        onClick={onClick}
      >
        <CardContent className="p-6 flex flex-col justify-between h-full">
          {/* Project Image with enhanced gradient overlay */}
